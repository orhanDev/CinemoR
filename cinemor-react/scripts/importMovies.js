...
const MOVIES_IMAGES_PATH = path.join(__dirname, '..', 'public', 'images', 'movies');
const OUTPUT_JSON = path.join(__dirname, '..', 'movies-data.json');
...
function findPosterImage(folderPath) {
  const files = fs.readdirSync(folderPath);
  const imageExtensions = ['.jpg', '.jpeg', '.png', '.JPG', '.JPEG', '.PNG'];
  
  for (const file of files) {
    const ext = path.extname(file);
    if (imageExtensions.includes(ext) && !file.includes('Textdokument')) {
...
async function importMovies() {
  ...
  for (const folder of folders) {
    ...
    const posterPath = findPosterImage(folderPath);
    if (posterPath) {
      ...
      const copiedFileName = copyImage(posterPath, destFolder, fileName);
      
      if (copiedFileName) {
        movie.posterPath = `/images/movies/${movie.isComingSoon ? 'comingsoon' : 'nowshowing'}/${copiedFileName}`;
